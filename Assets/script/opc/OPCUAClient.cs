...
namespace OPCClientInterface
{
    public class OPCUAClient
    {
        ...
        public async Task<bool> ConnectServer(string serverUrl)
        {
            ...
            {
                throw new Exception(ex+"连接失败，请检查网址是否正确！");
            }
            if (m_session == null)
            {
                throw new Exception("连接失败，请检查网址是否正确！");
            }
            return true;
        }
        ...
        internal void AddSubscription(string key, string tag, Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback)
        {
        ...
        /// <param name="tags">节点名称数组</param>
        /// <param name="callback">回调方法</param>
        internal void AddSubscription(string key, string[] tags, Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback)
        {
        ...
        internal IEnumerable<DataValue> ReadHistoryRawDataValues(string tag, DateTime start, DateTime end, uint count = 1, bool containBound = false)
        {
        ...
        /// <param name="containBound">是否包含边界</param>
        /// <returns>读取的数据列表</returns>
        internal IEnumerable<T> ReadHistoryRawDataValues<T>(string tag, DateTime start, DateTime end, uint count = 1, bool containBound = false)
        {
            HistoryReadValueId m_nodeToContinue = new HistoryReadValueId()
            {
                NodeId = new NodeId(tag),
        ...
        {
            m_ConnectComplete?.Invoke(this, e);
        }
        /// <summary>
        /// 检查状态返回值
        /// </summary>
